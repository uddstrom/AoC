...
import { printMaze, printPath } from './misc.js';

export function parser(input) {
    return input.split('\n').map((row) => row.split(''));
}

export function solvePartTwo(verbose = false) {
    var startTime = Date.now();

    //const PUZZLE_INPUT_PATH = `${getPath(import.meta.url)}/puzzle_input_test_3`;
    const PUZZLE_INPUT_PATH = `${getPath(import.meta.url)}/puzzle_input_part2`;
